...
/**
 * Computes the longest paths in a Directed Acyclic Graph (DAG) from a given source vertex.
 * Uses topological sorting and dynamic programming to calculate distances efficiently.
...
public class DAGLongestPath {
    ...
    /**
     * Computes longest paths from the source vertex to all reachable vertices.
     * @param graph the DAG
    ...
    public PathResult longestPaths(Graph graph, int source, Map<String, Integer> weights) {
        ...
        for (int u : order) {
            if (dist.get(u) == Integer.MIN_VALUE) continue; // unreachable
        ...
        metrics.stop();
        return new PathResult(dist, parent, true);
    }
